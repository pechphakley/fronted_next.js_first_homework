...
async function getSingleBlog(id: string) {
  try {
    // Corrected DummyJSON API route path with /posts/
    ...
    console.error("Error fetching single post on server:", err);
    return null;
  }
}
...
export default async function BlogSlug({ params }: PageProps) {
  ...
  // Fallback state if the post ID doesn't exist or API fails
  if (!blog) {
    return (
